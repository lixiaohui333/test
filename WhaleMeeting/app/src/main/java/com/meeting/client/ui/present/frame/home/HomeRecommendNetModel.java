...
public class HomeRecommendNetModel implements NetTaskModel {


    public HomeRecommendNetModel() {
        createRetrofit();
    }
    ...
    private void createRetrofit() {
        retrofit = new Retrofit.Builder().baseUrl(HTTP_HOST)
    ...
    public Disposable execute(final LoadTaskCallBack taskCallback,String... params) {
        ...
        Disposable disposable = weatherService.getHomeRecommend("0").subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<BaseHR<RecommendDomain>>() {
                    @Override
                    public void accept(BaseHR<RecommendDomain> baseDataListBaseHR) throws Exception {
