...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	/** 上拉加载是否可用 */
	protected boolean mPullLoadEnabled = true;
	/** 是否滑动到底部自动加载 */
	private boolean mAutoLoadOnBottom = false;
	/** 是否截断touch事件 */
	...
	/** 未满屏时是否允许上下拉 */
	private boolean mAwaysPullUpDownEnable = false;
	...
	private PullState mCurrentAction = PullState.NONE;
	...
	*  false时，仅在超过一屏才可上拉
	*/
	public void setAwaysPullUpDownEnable(boolean enable){
		mAwaysPullUpDownEnable = enable;
	}
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading()) {
				...
				if (isPullRefreshEnabled() && isReadyForPullDown() && isToPullDown(deltaY)) {
				...
				} else if (isPullLoadEnabled() && !isAutoLoadOnBottom() && isReadyForPullUp() && isToPullUp(deltaY)) {
					// 原理如上
					mIsHandledTouchEvent = (Math.abs(getScrollYValue()) > 0 || deltaY < -0.5f);
					if (mIsHandledTouchEvent) {
						mRefreshableView.onTouchEvent(event);
	...
	public final boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (mIsHandledTouchEvent) {
				mIsHandledTouchEvent = false;
				if(mAwaysPullUpDownEnable){
					// 当第一个显示出来时
					if (isReadyForPullDown() && mCurrentAction == PullState.PULL_DOWN) {
						// 调用刷新
						...
						resetHeaderLayout();
					}else if (isReadyForPullUp()&& mCurrentAction == PullState.PULL_UP) {
						// 加载更多
	...
	@Override
	public void setAutoLoadOnBottomEnabled(boolean AutoLoadOnBottomEnabled) {
		mAutoLoadOnBottom = AutoLoadOnBottomEnabled;
	}
	...
	* @return boolean
	*/
	private boolean isToPullDown(float deltaY){
		if(mAwaysPullUpDownEnable){
			if((deltaY >0 && getScrollYValue()<=0)||(getScrollYValue()<0 && deltaY < 0)){
				mCurrentAction = PullState.PULL_DOWN;
				return true;
			}else{
				return false;
			}
	...
	* @return boolean
	*/
	private boolean isToPullUp(float deltaY){
		if(mAwaysPullUpDownEnable){
			if((deltaY <0 && getScrollY()>=0)||(deltaY>0 && getScrollY()>0)){
				mCurrentAction = PullState.PULL_UP;
				return true;
			}else{
				return false;
			}
	...
	protected boolean isPullLoading() {
		return (mPullUpState == ILoadingLayout.State.LOADING);
	}
	...
	protected boolean isRefreshing(){
		...
		return (mPullDownState == ILoadingLayout.State.REFRESHING || mPullUpState == ILoadingLayout.State.LOADING);
	}
	...
	protected void startLoading() {
		...
		mPullUpState = ILoadingLayout.State.LOADING;
		onStateChanged(ILoadingLayout.State.LOADING, false);

		if (null != mFooterLayout) {
			mFooterLayout.setState(ILoadingLayout.State.LOADING);
		}
